...
if not connection_string:
    ...
    # sitl = dronekit_sitl.start_default()               ##original
    sitl = dronekit_sitl.SITL()
    connection_string = sitl.connection_string()
    sitl.download('copter', '3.3', verbose=True)
    sitl_args = ['-I0', '--model', 'quad', '--home=29.58487490500391,-98.61565014236473,0,180']
...
class Drone(object):
    def __init__(self, server_enabled=True):
        ...
        self.vehicle.add_attribute_listener('airspeed', self.airspeed_callback)
        '''
        new
        '''
        self.targetalt = 30 
    ...
    def takeoff(self):
        ...
        self.vehicle.simple_takeoff(self.targetalt)  ###### was 30
...
''')
    ...
    def goto(self, location, relative=None):
        ...
        while True:
            ...
            if vehicle.location.global_relative_frame.alt>=self.targetalt*0.95:
        ...
        '''
        new
        problem: will not update altitude while the loop is running***
        problem2: must manually hit track now****
        problem3: takeoff does not reach 30 before going to new waypoint after RTL
        '''
        distancetopoint = 20
        while distancetopoint > 5:
            ...
            # print(location[0])
            # print(location[1])
            dlat = float(location[0]) - self.vehicle.location.global_frame.lat
            dlong = float(location[1]) - self.vehicle.location.global_frame.lon
            distancetopoint = math.sqrt((dlat*dlat) + (dlong*dlong)) * 1.113195e5
            time.sleep(2)
    ...
    def airspeed_callback(self, vehicle, name, airspeed):
...
class Templates:
    ...
    def get_options(self):
        return {'width': 670,
                ...
                'zoom': 16,
                'format': 'png',
                'access_token': 'pk.eyJ1Ijoic2Z5MDY0IiwiYSI6ImNsYWhqYjQwdTBjbWwzcW1rMGF0dGQ2ZW8ifQ.uGlMHjEumKRIn_HJs_3pLg',
                'mapid': 'mapbox.satellite', #mapbox.satellite  #mapbox.mapbox-streets-v8 
...
class DroneDelivery(object):
    ...
    def vehicle(self):
        return dict(position=self.drone.get_location(), altitude=self.drone.get_altitude(), airspeed=self.drone.get_airspeed())
    ...
    def track(self, lat=None, lon=None):
        ...
        return self.templates.track(self.drone.get_location(), self.drone.get_altitude(), self.drone.get_airspeed())
